...
- **`main`** (this branch): the ML + simulation expansion — labs/imaging generators, A1–A9 / B1–B14 / E1–E12 pipelines, `migrations/`, audit scripts. Plus a **multi-portal frontend** (`frontend/`, React + Vite + Tailwind — Doctor / Surveillance / Federated & Privacy / AI Operations portals per the integration spec; see `RUNNING.md` and `frontend/RUNNING.md`) and a **FastAPI backend** (`backend/app/`, see `backend/RUNNING.md`) that serves the whole `/api/*` surface (spec section 7) against a Supabase/PostgreSQL schema (`backend/migrations/002_supabase_app_schema.sql`, loaded by `backend/seed_supabase.py`). The frontend still defaults to labelled sample fixtures (`frontend/src/mocks/*.js`); `npm run dev:live` (`VITE_USE_MOCK=0`) flips `frontend/src/services/*` to the live backend. The backend needs `DATABASE_URL` in the repo-root `.env`; the treatment-advisor `/api/clinical/treatment-advisor` endpoint uses a DB-config-backed heuristic engine (`backend/app/services/advisor_engine.py`, labelled `engine: "heuristic_fallback"`) when the ML E1–E12 pipeline can't run, with E12 weights kept in Python.
